...
export default class Gateway {
  ...
  constructor(dbFile: string, wipe: boolean) {
    ...
    if (wipe) {
      this.dropTables().then(() => this.createTable())
    }
    else
      this.createTable()
  }
  ...
  async createTable() {
    ...
    let tables = [
      "CREATE TABLE IF NOT EXISTS Users (`userid` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,`name` TEXT(1000) NOT NULL,`username` TEXT(1000) NOT NULL,`email` TEXT(1000) NOT NULL,`score` INTEGER(20)  NOT NULL DEFAULT (0),`totalGames` INTEGER(20)  NOT NULL DEFAULT (0),`private` INTEGER(1) DEFAULT (0),CONSTRAINT `username_uq` UNIQUE (`username`),CONSTRAINT `email_uq` UNIQUE (`email`));",
      ...
      "CREATE TABLE IF NOT EXISTS MatchAcceptances (`matcchacceptanceid` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,`matchid` INTEGER(20)  NOT NULL,`userid` INTEGER(20),`status` INTEGER(20)  NOT NULL DEFAULT (0),`msg` TEXT(1000),CONSTRAINT `matchid_fk` FOREIGN KEY (`matchid`) REFERENCES matchs(`matchid`),CONSTRAINT `userid_fk` FOREIGN KEY (`userid`) REFERENCES users(`userid`));",
