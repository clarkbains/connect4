   ...
   // db: sqlite3.Database
    ...
    generateSelect(opts: SqlGeneratorOpts): PromisifiedSqlOpts {
        if (!opts.whereBinds && !opts.whereClause) {
            let queryParts: string[] = []
            let binds: (string | number)[] = []
            console.log("started select", opts.obj)
            for (let key of Object.keys(opts.obj)) {
                if (opts.obj[key] != undefined) {
                    binds.push(opts.obj[key])
                    queryParts.push(`\`${key}\`=?`)
...
export interface CRUD {
    insert(db:sqlite3.Database): Promise<void>
    select(db:sqlite3.Database,whereClause?: string, whereBinds?: any[]): Promise<DatabaseModel>
    update(db:sqlite3.Database,whereClause?: string, whereBinds?: any[]): Promise<void>
    delete(db:sqlite3.Database,whereClause?: string, whereBinds?: any[]): Promise<void>
}